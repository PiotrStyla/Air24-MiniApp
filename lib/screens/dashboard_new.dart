...
class _ClaimDashboardScreenState extends State<ClaimDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildAnalyticsCard(DashboardStats stats) {
    // Calculate success rate if there are any completed claims
    final successRate = stats.totalClaims > 0 
        ? stats.completedClaims / stats.totalClaims 
        : 0.0;
  ...
  Widget _buildClaimsList(List<Claim> claims) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        // Get the viewmodel from the provider and refresh
        final viewModel = Provider.of<ClaimDashboardViewModel>(context, listen: false);
        return viewModel.refreshDashboard();
      },
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              onPressed: () {
                ...
                Navigator.of(context).pushNamed('/quick-claim');
              },
  ...
  Widget _buildErrorView(String errorMessage) {
    return Semantics(
      label: 'Error loading claims',
      value: errorMessage,
      button: true,
      enabled: true,
      onTapHint: 'Retry loading claims',
      child: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                semanticsLabel: 'Error loading claims',
              ),
              const SizedBox(height: 8),
              Text(
                errorMessage,
              ...
              const SizedBox(height: 24),
              ElevatedButton.icon(
                onPressed: () => _viewModel.refreshDashboard(),
                icon: const Icon(Icons.refresh),
  ...
  void _navigateToClaimDetail(String claimId) {
    Navigator.of(context).push(
      ...
      // to reflect any changes made to the claim
