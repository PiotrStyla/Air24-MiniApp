...
def is_eligible_for_compensation(flight, hours_ago=None, debug=False):
    """
    ...
    try:
        ...
        if debug:
            debug_info["is_cancelled"] = is_cancelled
            debug_info["is_delayed"] = is_delayed
        ...
        if is_cancelled or is_delayed:
            ...
            if debug:
                debug_info["departure_airport"] = departure_airport
                debug_info["arrival_airport"] = arrival_airport
            ...
            if debug:
                debug_info["is_eu_flight"] = is_eu_flight
                
            if is_eu_flight:
                if debug:
                    debug_info["eligible"] = True
                    debug_info["reason"] = "EU flight with delay/cancellation"
                    return True, debug_info
                return True
            else:
                if debug:
                    debug_info["eligible"] = False
                    debug_info["reason"] = "Not an EU flight"
                    return False, debug_info
                return False
        else:
            if debug:
                debug_info["eligible"] = False
                debug_info["reason"] = "Not delayed or cancelled"
                return False, debug_info
            return False
...
def analyze_eligibility_for_all_flights():
    """Analyze eligibility for all flights in the database"""
    ...
    # EU airports for reference
    eu_airports = ['MAD', 'CDG', 'FRA', 'AMS', 'FCO', 'LHR', 'MUC', 'BCN', 'ATH', 'VIE', 'WAW', 'DUB', 'BRU', 'LIS', 'HEL', 'PRG', 'CPH', 'BUD', 'ARN', 'TXL', 'OTP', 'SOF', 'LJU', 'RIX', 'VNO', 'TLL']
    
    # Analyze each flight
    for i, flight in enumerate(flights):
        # Debug data
        is_eligible, debug_data = is_eligible_for_compensation(flight, debug=True)
        
        # Check cancellation
        if debug_data.get("is_cancelled", False):
            cancelled_count += 1
        
        # Check delay
        if debug_data.get("is_delayed", False):
            delayed_count += 1
        
        # Check EU airports
        if debug_data.get("is_eu_flight", False):
            eu_flight_count += 1
        
        # If eligible, add to eligible flights
        if is_eligible:
            eligible_flights.append({
                'flight': debug_data.get('flight', ''),
                'reason': debug_data.get('reason', '')
            })
        
        # First 10 flights for detailed analysis
        if i < 10:
            issues.append(debug_data)
    ...
    return {
        ...
        "eu_flights": eu_flight_count,
        "eligible_flights_count": len(eligible_flights),
        "eligible_flights": eligible_flights,
        "issues": issues,
        "recommendations": get_recommendations(cancelled_count, delayed_count, eu_flight_count, total_flights)
    }

def get_recommendations(cancelled_count, delayed_count, eu_flights, total_flights):
    """Get recommendations for fixing eligibility issues"""
    recommendations = []
    
    if cancelled_count == 0:
        recommendations.append("No cancelled flights found. Add some flights with 'CANCELLED' status.")
    
    if delayed_count == 0:
        recommendations.append("No flights with delay >= 180 minutes found. Add flights with higher delay values.")
    
    if eu_flights == 0:
        recommendations.append("No flights with EU airports found. Add flights with at least one EU airport (departure or arrival).")
    
    if cancelled_count > 0 and delayed_count > 0 and eu_flights > 0 and len(recommendations) == 0:
        recommendations.append("Data has cancelled/delayed EU flights but none are eligible. Check data structure and ensure fields match expected format.")
        recommendations.append("Check that airport IATA codes match exactly with the EU airport list.")
...
def application(environ, start_response):
    ...
    try:
        ...
        # NEW DIAGNOSTIC ENDPOINT: check eligibility for all flights
        elif path == '/debug_eligibility':
            # Run eligibility diagnostics
            results = analyze_eligibility_for_all_flights()
